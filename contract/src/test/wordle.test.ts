...
describe('P2P ZK Wordle Contract Tests', () => {
  ...
  describe('Turn Management', () => {
    test('Player 1 can make first guess', () => {
      ...
      expect(p1State.game_state).toBe(GAME_STATE.p2_turn);
      expect(p1State.p1_guess_count).toBe(1n);
      expect(wordToString(p1State.current_guess)).toBe("SALTY");
    });
    ...
    test('Player 2 verifies guess and makes counter-guess', () => {
      ...
      expect(p2State.last_guess_result.is_some).toBe(true);
      
      // Verify the guess result for "SLIME" vs "SLATE"
      const result = p2State.last_guess_result.value;
    ...
    test('Player 1 verifies Player 2 guess', () => {
      ...
      simulator.as('p2').turn_player2(stringToWord("CREEP"));
      
      // Player 1 verifies Player 2's guess
      const p1State = simulator.as('p1').verify_guess();
      
      expect(p1State.game_state).toBe(GAME_STATE.p1_turn);
      expect(p1State.last_guess_result.is_some).toBe(true);
      
      // Verify the guess result for "CREEP" vs "CRANE"
      const result = p1State.last_guess_result.value;
      expect(result.first_letter_result).toBe(2n);  // C - correct position
      expect(result.second_letter_result).toBe(2n); // R - correct position
      expect(result.third_letter_result).toBe(1n);  // E - in CRANE but wrong position  
      expect(result.fourth_letter_result).toBe(1n); // E - in CRANE but wrong position
      expect(result.fifth_letter_result).toBe(0n);  // P - not in CRANE
    });
    ...
    test('Reject guess when not player turn', () => {
      ...
      }).toThrow("failed assert: Not player 2's turn");
    });

    test('Reject verification when not player turn', () => {
      const simulator = createGame();
      ...
      simulator.as('p1').turn_player1(stringToWord('CRANE'));
      
      // Now it's Player 2's turn, so Player 1 can't verify yet
      expect(() => {
        simulator.as('p1').verify_guess();
      }).toThrow("failed assert: Not player 1's turn to verify");
    });
  });
  ...
  describe('Win Conditions', () => {
    ...
    test('Player 2 wins by guessing Player 1 word correctly', () => {
      ...
      // Player 1 verifies the correct guess
      const p1State = simulator.as('p1').verify_guess();
    ...
    test('Game ends in draw after 6 guesses each', () => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        // Player 2 verifies and guesses (if not the last round)
        if (i < 5) {
          simulator.as('p2').turn_player2(stringToWord(wrongGuesses[i]));
          simulator.as('p1').verify_guess();
        } else {
          // Last round - Player 2 makes final guess
          const p2State = simulator.as('p2').turn_player2(stringToWord(wrongGuesses[i]));
  ...
  describe('Player Identity Verification', () => {
    ...
    test('Reject verification from wrong player', () => {
      ...
      // Player 2 tries to verify instead of Player 1
      expect(() => {
        simulator.as('p2').verify_guess();
      }).toThrow("failed assert: You are not player 1");
    });
  });
